...
def poly_mul(f, g):
    ...
    if Lf <= 16 or Lg <= 16:
        ...
        return (np.convolve(f.astype(np.uint64), g.astype(np.uint64)) % mod).astype(int)
    fft = np.fft.rfft
    ifft = np.fft.irfft
    ...
    gl = g & (1 << 15) - 1
    gh = g >> 15
    x = (ifft(fft(fl, fft_len) * fft(gl, fft_len))[:L] + 0.5).astype(int) % mod
    y = (ifft(fft(fl + fh, fft_len) * fft(gl + gh, fft_len))[:L] + 0.5).astype(
        int
    ) % mod
    z = (ifft(fft(fh, fft_len) * fft(gh, fft_len))[:L] + 0.5).astype(int) % mod
...
def poly_inv(fps, n=None):
    ...
    if n is None:
        n = len(fps)
    res = np.zeros(1 << (n - 1).bit_length(), dtype=int)
    ...
    while i < n:
        ...
        res[:i] = (
            (res[:i] << 1)
            - poly_mul(poly_mul(res[: i >> 1], res[: i >> 1]), fps[:i])[:i]
        ) % mod
    return res[:n]
...
def lagrange_interpolation(X, Y, mod):
    ...
    for x, y in zip(X, Y):
        ...
        for i, f_i in enumerate(f[:n]):
            res[i] += (
                f_i * y * denom_inv
            )  # % mod  # mod が大きいと 64bit に収まらなくなるのでひとつずつ mod 取った方がいいか？
...
def polynomial_interpolation(xs, ys):
    ...
    k = 1 << bit
    fpss = np.zeros((bit + 1, n + 1), dtype=int)
    fpss[0, :n] = -xs % mod
    for i in range(bit):
        step = 2 << i
        half = step >> 1
    ...
    fpss2 = np.zeros((bit + 1, k + 1), dtype=int)
    fpss2[bit, :n] = poly_differential(f)
    for i in trange(bit - 1, threshold - 1, -1):
        step = 2 << i
        half = step >> 1
        for j in range(0, n, step):
            if j + half >= n:
    ...
    xs_cumprod = np.ones_like(xs)
    f = np.zeros_like(xs)
    for i in trange(1 << threshold):
        f += fpss2[threshold, i : k : 1 << threshold, None] * xs_cumprod % mod
        xs_cumprod *= xs
        xs_cumprod %= mod
    f = f.ravel()
    for j in trange(n):
        fpss2[0, j] = int(ys[j]) * int(pow(int(f[j]), mod - 2, mod)) % mod
    for i in trange(bit):
        step = 2 << i
        half = step >> 1
        for j in range(0, k, step):
...
def test():
    ...
    y = multipoint_evaluation(a, x)
    print(f"ys={y}")
    a2 = polynomial_interpolation(x, y)
    print(f"a2={a2}")
...
def main():
    ...
    X = np.array(sys.stdin.buffer.readline().split(), dtype=int)
    Y = np.array(sys.stdin.buffer.readline().split(), dtype=int)
