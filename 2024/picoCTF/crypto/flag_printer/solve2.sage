...
#         return (np.convolve(f.astype(np.uint64), g.astype(np.uint64)) % mod).astype(
#             int
#         )
#     fft = np.fft.rfft
#     ifft = np.fft.irfft
...
def poly_mul(f, g):
    f = PR(list(f))
    g = PR(list(g))
    # print(list(map(int, f * g)))
    return np.array(list(f * g), dtype=int)
...
def main():
    ...
    Ans = polynomial_interpolation(X, Y) % mod
