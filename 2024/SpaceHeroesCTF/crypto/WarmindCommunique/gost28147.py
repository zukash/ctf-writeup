...
def xcrypt(seq, sbox, key, ns):
    ...
    w = bytearray(key)
    x = [
        w[0 + i * 4] | w[1 + i * 4] << 8 | w[2 + i * 4] << 16 | w[3 + i * 4] << 24
        for i in range(8)
    ]
    n1, n2 = ns
    for i in seq:
...
def ecb(key, data, action, sbox=DEFAULT_SBOX):
    ...
    for i in xrange(0, len(data), BLOCKSIZE):
        result.append(ns2block(action(sbox, key, block2ns(data[i : i + BLOCKSIZE]))))
...
def cbc_encrypt(key, data, iv=8 * b"\x00", pad=True, sbox=DEFAULT_SBOX):
    ...
    for i in xrange(0, len(data), BLOCKSIZE):
        ciphertext.append(
            ns2block(
                encrypt(
                    sbox, key, block2ns(strxor(ciphertext[-1], data[i : i + BLOCKSIZE]))
                )
            )
        )
...
def cfb_decrypt(key, data, iv=8 * b"\x00", sbox=DEFAULT_SBOX, mesh=False):
    ...
    for i in xrange(BLOCKSIZE, len(data), BLOCKSIZE):
        if (
            mesh
            and (i - BLOCKSIZE) >= MESH_MAX_DATA
            and (i - BLOCKSIZE) % MESH_MAX_DATA == 0
        ):
