...
r = int(_sage_const_0x73eda753299d7d483339d80809a1d80553bda402fffe5bfeffffffff00000001 )

F = GF(q, x, x)
F2 = GF(q**_sage_const_2 , x, x**_sage_const_2  + _sage_const_1 , names=('x',)); (x,) = F2._first_ngens(1)
...
class Point(namedtuple("_Point", "x y")):
    ...
    def is_valid(self):
        return self.is_inf() or self.x ** _sage_const_3  + down(self.x.parent(), _sage_const_4 *x + _sage_const_4 ) == self.y ** _sage_const_2 

    @property
    def inf(self):
        return type(self)(self.x.parent()(_sage_const_0 ), self.y.parent()(_sage_const_0 ))

    def is_inf(self):
        return self.x == self.y == _sage_const_0 
    ...
    def __add__(self, o):
        ...
        if self != o and self.x == o.x:
            return self.inf
        if self == o:
            l = (_sage_const_3 *(self.x**_sage_const_2 ))/(_sage_const_2 *self.y)
        else:
            l = (self.y - o.y)/(self.x - o.x)
        nx = l**_sage_const_2  - self.x - o.x
    ...
    def __mul__(self, scalar):
        if scalar < _sage_const_0 :
            self = -self
            scalar = -scalar
        res = self.inf
        while scalar:
            if scalar & _sage_const_1 :
                res = res + self
            self = self + self
            scalar >>= _sage_const_1 
        return res

G1 = Point(F(_sage_const_4 ), F(_sage_const_0x0a989badd40d6212b33cffc3f3763e9bc760f988c9926b26da9dd85e928483446346b8ed00e1de5d5ea93e354abe706c )) * _sage_const_0x396c8c005555e1568c00aaab0000aaab 
G2 = Point(F2(_sage_const_2 ), F2(_sage_const_0x013a59858b6809fca4d9a3b6539246a70051a3c88899964a42bc9a69cf9acdd9dd387cfa9086b894185b9a46a402be73  + _sage_const_0x02d27e0ec3356299a346a09ad7dc4ef68a483c3aed53f9139d2f929a3eecebf72082e5e58c6da24ee32e03040c406d4f *x)) * _sage_const_0x5d543a95414e7f1091d50792876a202cd91de4547085abaa68a205b2e5a7ddfa628f1cb4d9e82ef21537e293a6691ae1616ec6e786f0c70cf1c38e31c7238e5 
...
def exchange_real():
    ...
    b = randrange(r)
    P_B = G2 * b
    P_C = [P_B * a, P_A * b][randrange(int(_sage_const_2 ))]
...
def exchange_fake():
    ...
    P_B = G2 * b
    c = randrange(r)
    P_C = [G1, G2][randrange(int(_sage_const_2 ))] * c
    return format_output(P_A, P_B, P_C)

if __name__ == "__main__":
    with open(sys.argv[_sage_const_1 ], "rb") as f:
        flag = f.read()
    res = []
    for byte in flag:
        for bit in map(int, bin(byte)[_sage_const_2 :].zfill(_sage_const_8 )):
            if bit:
