...
def print_encrypted(message, iv, key):
    ...
    payload = (
        encryptor.update(message + b"\x00" * (16 - len(message) % 16))
        + encryptor.finalize()
    )
...
def main():
    ...
    with open("server-ecdhkey.pem", "rb") as server_key_file:
        server_key = serialization.load_pem_private_key(
            server_key_file.read(), None, default_backend()
        )
    ...
    client_cert = x509.load_pem_x509_certificate(client_cert_content.encode("utf-8"))
    ...
    server_ephemeral_key = ec.generate_private_key(ec.SECP256R1(), default_backend())
    server_ephemeral_public_key = server_ephemeral_key.public_key()
    print("Server ephemeral key:")
    print(
        server_ephemeral_public_key.public_bytes(
    ...
    while True:
        ...
        print_encrypted("Gimme your secrets:", server_ephemeral_random, derived_key)
        ...
        with open(path, "rb+") as f:
            ...
            f.write(secret)
            print_encrypted(
                "Saved! Previous secret reference: " + prev_hash,
                server_ephemeral_random,
                derived_key,
            )
