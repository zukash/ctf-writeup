from Crypto.Util.number import *

from itertools import product
from tqdm import tqdm


def solve(e, n):
    cf = continued_fraction(e / n)
    cand = []
    for conv in cf.convergents():
        k = conv.numerator()
        d = conv.denominator()
        if k == 0:
            continue
        # d * e = k * (n - p - q + 1) + 1
        # p + q = n - (d * e - 1) / k + 1
        pq = n - (d * e - 1) / k + 1
        if not pq.is_integer():
            continue
        # (x - p) * (x - q) = 0 の解が p,q
        # x^2 - (p + q) * x + n = 0 の解が p,q
        cand.append(d)

        # x = var("x")
        # f = x ^ 2 - pq * x + n
        # sol = f.roots()
        # if len(sol) != 2:
        #     continue
        # p, q = sol[0][0], sol[1][0]
        # if not (p.is_integer() and q.is_integer()):
        #     continue
        # if not (is_prime(p) and is_prime(q)):
        #     continue
        # return p, q, d
    return cand


PKEY = 
ENC = 

r, nbit = 8, 1024
n_ = int(f"{PKEY:b}"[: nbit * 2 - r], 2) << r
e_ = int(f"{PKEY:b}"[nbit * 2 - r :], 2) << r
enc_ = ENC << r


def has_small_factor(n):
    for p in Primes()[: 10 ** 5]:
        if n % p == 0:
            return True
    return False


def decrypt(c, p, q, e):
    phi = (p - 1) * (q - 1)
    if GCD(e, phi) != 1:
        return b"\x00"
    d = pow(e, -1, phi)
    m = pow(c, d, p * q)
    return long_to_bytes(m)


N = []
for s in tqdm(range(1 << r)):
    n = n_ + s
    if not has_small_factor(n):
        N.append(n)


for n, t in tqdm(list(product(N, range(1 << r)))):
    e = e_ + t
    cand = solve(e, n)
    for d in cand:
        for u in range(1 << r):
            enc = enc_ + u
            flag = long_to_bytes(pow(enc, d, n))
            try:
                flag = flag.decode()
                print(flag)
                if flag.is_printable():
                    print(flag)
                    exit()
            except:
                pass

"""
 32%|███████████████████████████████████████████████████████████████████████████▊                                                                                                                                                                   | 975/3072 [04:30<11:23,  3.07it/s] 64%|████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████                                                                                      | 1963/3072 [09:16<06:03,  3.05it/s]aIr_pr3s5urE_d!Ff3rEn7i4L_8eTw3eN_ArEa5!
100%|██████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████| 3072/3072 [14:02<00:00,  3.65it/s]
"""
