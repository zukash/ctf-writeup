...
class DESClass:
    ...
    def encryptDict(
        self, key: bytes, plaintext_dx: dict[bytes, bytes], cores: int
    ) -> dict[bytes, bytes]:
        """
        ...
        with Pool(cores) as pool:
            for ct_label, ct_value in pool.istarmap(
                encryptDictHelper, iterable, chunksize=chunk
            ):
...
if __name__ == "__main__":
    des = DESClass()
    key = des.keyGen(16)
    encrypted_db = des.encryptDict(key, {b"hoge": b"fuga", b"piyo": b"piko"}, 1)
    token = des.tokenGen(key, b"piyo")
    print(encrypted_db)
    print(des.search(token, encrypted_db))
