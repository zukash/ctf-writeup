...
_sage_const_1024 = Integer(1024); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_3 = Integer(3)
import os
...
class Privkey:
    ...
    @classmethod
    def generate(cls, pbits: int = _sage_const_1024 ) -> "Privkey":
        p = getPrime(pbits)
        q = getPrime(pbits)
        n = p * q
        Q = QuaternionAlgebra(Zmod(n**_sage_const_2 ), -_sage_const_1 , -_sage_const_1 )
        g = _sage_const_1  + Q.random_element() * n
        l = lcm(p - _sage_const_1 , q - _sage_const_1 )
...
class Paillier:
    ...
    def encrypt(self, m: int):
        n = self.pubkey.n
        g = self.pubkey.g
        assert _sage_const_1  <= m < n
        return g**m * pow(randbelow(n**_sage_const_2 ), n, n**_sage_const_2 )
    ...
    def L(self, u: QuaternionAlgebraElement_generic):
        ...
        i, j, k = Q.gens()
        return (
            int(u[_sage_const_0 ] - _sage_const_1 ) // n
            + int(u[_sage_const_1 ]) // n * i
            + int(u[_sage_const_2 ]) // n * j
            + int(u[_sage_const_3 ]) // n * k
        )
    ...
    def decrypt(self, c):
        ...
        Q = g.parent()
        i, j, k = Q.gens()
        tmp = self.L(c**l) * self.L(g**l) ** -_sage_const_1 
        return (
            int(tmp[_sage_const_0 ] % n)
            + int(tmp[_sage_const_1 ] % n) * i
            + int(tmp[_sage_const_2 ] % n) * j
            + int(tmp[_sage_const_3 ] % n) * k
        )


if __name__ == "__main__":
    privkey = Privkey.generate(_sage_const_1024 )
